...
function UploadFormContent() {
  ...
  // Validate type
  const type =
    rawType === "karya" || rawType === "acara" || rawType === "blog"
      ? rawType
      : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Common validations
    if (!data.image_url) {
      return showToast("URL gambar wajib diisi!");
    }

    if (!/^https?:\/\/.+/i.test(data.image_url)) {
      return showToast(
        "URL gambar harus diawali dengan http:// atau https://"
      );
    }

    if (!data.title || data.title.length < 5) {
      return showToast("Judul terlalu pendek (minimal 5 karakter).");
    }
    ...
    if (type === "karya") {
      if (!data.description || data.description.length < 10) {
        return showToast(
          "Deskripsi terlalu singkat (minimal 10 karakter)."
        );
      }

      if (data.link && !/^https?:\/\//i.test(data.link)) {
        return showToast(
          "Link harus diawali dengan http:// atau https://"
        );
      }
    }
    ...
    if (type === "acara") {
      if (!data.location || data.location.length < 5) {
        return showToast(
          "Lokasi acara kurang jelas (minimal 5 karakter)."
        );
      }

      if (!data.time || !data.time.includes(":")) {
        return showToast(
          "Waktu harus mengandung titik dua ':' (contoh: 08:00 - Selesai)."
        );
      }
    ...
    if (type === "blog") {
      if (
        ...
        !/^[a-zA-Z\s]+$/.test(data.author)
      ) {
        return showToast(
          "Nama Penulis salah. Hanya boleh pakai huruf (minimal 3 huruf)."
        );
      }

      if (!data.readTime || !/min|menit/i.test(data.readTime)) {
        return showToast(
          "Waktu baca harus mengandung kata 'min' atau 'menit'."
        );
      }

      if (
        !data.excerpt ||
        data.excerpt.length < 15 ||
        data.excerpt.length > 200
      ) {
        return showToast(
          "Panjang cuplikan artikel harus antara 15 hingga 200 karakter."
        );
      }

      if (!data.content || data.content.length < 50) {
        return showToast(
          "Isi artikel terlalu pendek (minimal 50 karakter)."
        );
      }
    }
        ...
        <div className="max-w-md mx-auto text-center mt-10 p-8 bg-[#0c1322] border border-white/10 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-300">
          ...
          <form
            onSubmit={(e) => {
              ...
              if (
                password ===
                "PASSWORDUPLOADHMIFWEB2026CIHUYHMIFUNM"
              ) {
                setIsAuthorized(true);
                setAuthError("");
              } else {
          ...
          >
            ...
            {authError && (
              <p className="text-red-400 text-sm mb-4 font-medium animate-in slide-in-from-top-2">
                {authError}
              </p>
            )}
            ...
            <button
              type="submit"
              className="w-full bg-brand-600 hover:bg-brand-500 text-white font-bold py-3.5 rounded-xl transition-colors text-sm uppercase tracking-wide"
            >
              Buka Kunci
            </button>
          </form>
        ...
        <>
          <div className="mb-8 border-b border-white/10 pb-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <p className="text-slate-300">
              Silakan isi form di bawah ini dengan lengkap.
            </p>
          </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                    required
                  />

                  <p className="text-xs text-slate-400 mt-2">
                    Masukkan URL gambar dari mana saja, contoh GitHub
                    Raw, Pronto, ImageKit, atau CDN lainnya.
                  </p>

                  {formData.image_url && (
                    <div className="mt-4 border border-white/10 rounded-xl p-4 bg-[#0c1322]">
                ...
                {type === "karya" && (
                  <>
                    ...
                    <input
                      ...
                      value={formData.link || ""}
                      onChange={handleInputChange}
                      placeholder="Link Proyek (Opsional)"
                      className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                    />
                  </>
                )}
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <input
                        ...
                        value={formData.date || ""}
                        onChange={handleInputChange}
                        placeholder="Tanggal (Contoh: 15 Agustus 2026)"
                        className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                        required
                      />
                      ...
                      <input
                        ...
                        value={formData.time || ""}
                        onChange={handleInputChange}
                        placeholder="Waktu (Contoh: 08:00 - Selesai)"
                        className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                        required
                      />
                    </div>
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder="Lokasi (Contoh: Auditorium Kampus)"
                      className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                      required
                    />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <input
                        ...
                        onChange={handleInputChange}
                        placeholder="Kategori (Seminar/Kompetisi)"
                        className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                        required
                      />
                        ...
                        >
                          ...
                          >
                            {formData.status === "past"
                              ...
                              ? "Akan Datang"
                              : "Pilih Status (Akan Datang/Selesai)"}
                          </span>
                        ...
                        {isStatusOpen && (
                          <div className="absolute top-full left-0 right-0 mt-2 bg-[#1a2235] border border-white/10 rounded-xl overflow-hidden z-20 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
                            ...
                            >
                              ...
                              {formData.status === "upcoming" && (
                                <div className="w-2 h-2 rounded-full bg-brand-400 shadow-[0_0_8px_rgba(99,102,241,1)]" />
                              )}
                            </div>
                ...
                {type === "blog" && (
                  <>
                    <input
                      ...
                      value={formData.title || ""}
                      onChange={handleInputChange}
                      placeholder="Judul Artikel (Menarik & Jelas)"
                      className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors text-lg font-bold"
                      required
                    />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <input
                        ...
                        value={formData.category || ""}
                        onChange={handleInputChange}
                        placeholder="Kategori (Contoh: Teknologi, Opini)"
                        className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                        required
                      />
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        placeholder="Waktu Baca (Misal: 5 min read)"
                        className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                        required
                      />
                    </div>
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      placeholder="Cuplikan Artikel Singkat (Muncul di bawah judul)"
                      className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors"
                      rows={3}
                      required
                    />
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      placeholder="Isi Lengkap Artikel (Gunakan enter untuk paragraf baru)"
                      className="w-full bg-[#0c1322] border border-white/10 rounded-xl p-4 text-white focus:border-brand-500 outline-none transition-colors font-serif text-lg leading-relaxed min-h-[300px]"
                      required
                    />
                  </>
                )}
